...
Generate the code based on those instructions and these templates:
...
The code compiled near perfectly with the only issue being in the exception handling. ChatGPT compiled `catch (EmptyCollectionException e)` when it should have been `catch (InvalidDataException e)` however I was able to spot that with the naked eye.